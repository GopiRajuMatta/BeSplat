...
#         model_path, "render", name, "ours_{}".format(iteration), "renders")
#     gts_path = os.path.join(
#         model_path, "render", name, "ours_{}".format(iteration), "gt")
#     refs_path = os.path.join(
#         model_path, "render", name, "ours_{}".format(iteration), "ref")
#     depth_path = os.path.join(
#         model_path, "render", name, "ours_{}".format(iteration), "depth")
#     print(len(views))
...
#             torchvision.utils.save_image([ref,rgb], os.path.join(
#                 render_path, '{0:05d}'.format(idx+sample) + ".png"))
#             torchvision.utils.save_image(ref, os.path.join(
#                 refs_path, '{0:05d}'.format(idx+sample) + ".png"))
#             torchvision.utils.save_image(gt, os.path.join(
#                 gts_path, '{0:05d}'.format(idx+sample) + ".png"))
#             torchvision.utils.save_image(depth, os.path.join(
#                 depth_path, '{0:05d}'.format(idx+sample) + ".png"))
...
#                    [scene.getTrainCameras()[0]], gaussians, pipeline, background, optim_pose)
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool, optim_pose: bool):
    ...
    if not skip_train:
        render_set(dataset.model_path, "train", scene.loaded_iter,
                   [scene.getTrainCameras()[5]], gaussians, pipeline, background, optim_pose)
