...
def load_camera_pose(basedir, H, W, cubic):
    ...
    if cubic:
        ...
        print("poses_bounds_cubic_events.npy present")
    else:
        poses_arr = np.load(os.path.join(basedir, "poses_bounds.npy"))
        ev_poses_arr = np.load(os.path.join(basedir, "poses_bounds_events.npy"))
        print("poses_bounds_events.npy present")
...
def load_data(
    ...
    # print("RGB Exposure Time:", rgb_exp_time)
    # print("Poses Timestamps:", poses_ts)
    # print("Poses:", poses)
    # print("Event Poses:", ev_poses)
...
def regenerate_pose(
    ...
    if recenter:
        ...
        print("Recentered Poses:", poses)

    if spherify:
        poses, render_poses, bds = spherify_poses(poses, bds)
        print("Spherified Poses:", poses)
        print("Spherified Render Poses:", render_poses)
        print("Spherified Bounds:", bds)
    else:
        c2w = poses_avg(poses)
        print("Average Pose (c2w):", c2w)

        ## Get spiral
        up = normalize(poses[:, :3, 1].sum(0))
        print("Up Vector (Normalized):", up)

        # Find a reasonable "focus depth" for this dataset
        close_depth, inf_depth = bds.min() * 0.9, bds.max() * 5.0
        print("Close Depth:", close_depth)
        print("Infinite Depth:", inf_depth)

        dt = 0.75
        mean_dz = 1.0 / (((1.0 - dt) / close_depth + dt / inf_depth))
        focal = mean_dz
        print("Focal Distance:", focal)

        # Get radii for spiral path
        shrink_factor = 0.8
        zdelta = close_depth * 0.2
        print("Z Delta:", zdelta)

        tt = poses[:, :3, 3]  # ptstocam(poses[:3,3,:].T, c2w).T
        print("Translation Vectors (tt):", tt)

        rads = np.percentile(np.abs(tt), 90, 0)
        print("Radii (90th percentile):", rads)

        c2w_path = c2w
        N_views = 120
        N_rots = 2
        print("Number of Views:", N_views)
        print("Number of Rotations:", N_rots)
        ...
        if path_zflat:
            ...
            N_rots = 1
            N_views /= 2
            print("Z Location Adjustment (path_zflat):", zloc)
            print("Adjusted Radii (path_zflat):", rads)
            print("Adjusted Number of Rotations (path_zflat):", N_rots)
            print("Adjusted Number of Views (path_zflat):", N_views)

        # Generate poses for spiral path
        render_poses = render_path_spiral(
            c2w_path, up, rads, focal, zdelta, zrate=0.5, rots=N_rots, N=N_views
        )
        print("Render Poses (Spiral Path):", render_poses)

    render_poses = np.array(render_poses).astype(np.float32)
    print("Final Render Poses (as np.array):", render_poses)

    render_poses = torch.Tensor(render_poses)
    print("Final Render Poses (as torch.Tensor):", render_poses)
