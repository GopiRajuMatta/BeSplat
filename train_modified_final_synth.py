...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    opt.blur_sample_num = args.sharp_sample_num
    if not opt.deblur:
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if args.event_loss:
            ...
            for sample in events_ts:
                ...
                if iteration in saving_iterations:
                    render_path = os.path.join("output_spline", args.source_path.split("/")[-1]+"_"+str(args.index)+"_rgb_"+str(args.rgb_coeff)+"_event_"+str(args.event_coeff_real)+"_samples_"+str(args.sharp_sample_num),"train","train_{}".format(iteration),"event")
                    makedirs(render_path, exist_ok=True)
            ...
            if args.event_threshold>0:
                ...
                event_loss = mse_loss(render_brightness_diff, target_s)
            else:
...
def prepare_output_and_logger(args,args_):
    if not args.model_path:
        ...
        args.model_path = os.path.join("./output_spline/", args_.source_path.split("/")[-1]+"_"+str(args_.index)+"_rgb_"+str(args_.rgb_coeff)+"_event_"+str(args_.event_coeff_real)+"_samples_"+str(args_.sharp_sample_num))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type = str, 
                        default = "BeNeRF_Unreal", help='the dataset name')
    ...
    parser.add_argument("--event_time_window", action='store_false',
                        help='whether to use fixed windows or sliding window')
    parser.add_argument("--event_width", type=int, default=768,
                        help='the width of Event camera image')
    parser.add_argument("--event_height", type=int, default=480,
    ...
    parser.add_argument("--event_loss", action='store_false',
                        help='whether to compute event stream loss')
    parser.add_argument("--sharp_sample_num", type=int, default=19,
                        help='no. of sharp samples')
